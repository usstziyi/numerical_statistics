...
res = stats.ttest_ind(
        ...
        alternative='greater'  # 单侧检验：H1 只问"新药组降得比安慰剂组更多"，不管反向
)
# 顺序敏感：'greater' 检验的是"第一个参数的均值 > 第二个"，写反就变成检验相反假设
...
d = diff / sp
# 单侧检验的置信区间只给一个方向的下界，上界是 +inf
ci_low = res.confidence_interval(confidence_level=0.95).low
...
print(f"单侧 p-value = {p_value:.4f}")
print(f"差值 95% 单侧置信下界 = {ci_low:.2f} mmHg")
print(f"效应量 Cohen's d = {diff:.2f} / {sp:.2f} = {d:.2f}")

alpha = 0.05
if p_value < alpha:
    print(f"p = {p_value:.4f} < {alpha}  ->  拒绝 H0：新药降血压效果显著优于安慰剂")
else:
    print(f"p = {p_value:.4f} >= {alpha}  ->  不拒绝 H0：没有足够证据说明新药优于安慰剂")
...
# 单侧检验只关心右尾：t 比观测值更大的那部分面积就是 p
ax = axes[1]
xs = np.linspace(-5, 5, 1000)
ax.plot(xs, stats.t.pdf(xs, df), color='#4c72b0', linewidth=2,
        label=f't(df={df:.0f})')
ax.fill_between(xs, stats.t.pdf(xs, df), where=(xs > t_stat),
                color='#c44e52', alpha=0.4)
ax.axvline(t_stat, color='gray', linestyle=':', alpha=0.8)
ax.text(0.03, 0.95, f"t = {t_stat:.2f}, df = {df:.0f}\n单侧 p = {p_value:.4f}",
        transform=ax.transAxes, va='top', fontsize=10,
        bbox=dict(boxstyle='round', facecolor='white', edgecolor='gray', alpha=0.9))
ax.set_title('H0 成立时 t 的分布（右尾）')
ax.set_xlabel('t')
ax.set_ylabel('概率密度')
ax.legend(loc='upper right')
ax.grid(alpha=0.3)
